...
<![endif]-->
<title>爱.电影-场次列表</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 片场中心 <span class="c-gray en">&gt;</span> 片场管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加用户','/IMovie/pages/backpage/schedule-add','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加场次</a></span> <span class="r">共有数据：<strong>${scount}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="90">ID</th>
				<th width="130">电影名</th>
				<th width="100">放映日期</th>
				<th width="90">开始时刻</th>
				<th width="160">电影院</th>
				<th width="">所在大厅</th>
				<th width="90">所在城市</th>
				<th width="70" style="display: none;">状态</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		<tbody>
			<#list slist as sl>
				<tr class="text-c">
					<td><input type="checkbox" value="${sl.schedule_id}" name=""></td>
					<td>${sl_index+1}</td>
					<td><u  class="text-primary" >${sl.movie_name}</u></td>
					<td>${sl.schedule_date}</td>
					<td>${sl.schedule_begintime}</td>
					<td>${sl.cinema_name}</td>
					<td >${sl.hall_name}</td>
					<td>${sl.city_name}</td>
					<td class="td-status" style="display: none;"><span class="label label-success radius">已启用</span></td>
					<td class="td-manage"></a> <a title="删除" href="javascript:;" onclick="member_del(this,'${sl.schedule_id}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
			</#list>
		</tbody>
	</table>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type:"post",
			url:"/IMovie/schedule/deleteScheduleById",
			data:{"mid":id},
			dataType:"json",
...
function datadel(){
	...
	alert("选中要删除项！")
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			$.get("/IMovie/schedule/deleteScheduleByIds",{strids:params},function(data){
			list.each(function(){
				var id = "#tr"+$(this).val();
				$(id).remove();
				layer.msg('已删除!',{icon:1,time:1000});
