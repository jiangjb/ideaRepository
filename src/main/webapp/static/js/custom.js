...
function init_Home() {
    ...
    $('.score').raty({
        width:130, 
        score: 0,
        path: '/IMovie/static/images/rate/',
...
//var chooseSits = '';
var arraySites = new Array();//用来存已经选中的座位
...
//定义一个数组用来接收 已选中的座位
                    var arraySites1 = new Array();
                    $("#sits__checked22222 .choosen-place").each(function(){
                    	arraySites1.push($(this).text());
                    })
                    arraySites=arraySites1;
...
function book2Buy(sid){
	if(arraySites.length==0){
		alert("请选择座位！");
	}else{
		//alert(arraySites.length);
		//alert(sid);
		window.location.href="/IMovie/BookTwo/pickSeat?sid="+sid+"&arraySites="+arraySites;
	}
}
